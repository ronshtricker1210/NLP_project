...
Loads the model ONCE, then loops over one or more typo-rate configs (e.g. real10,real40,real70),
writing one JSONL line per question with the chain-of-thought, final answer, gold answer, and
all typo metadata. Designed to run inside a Slurm job that has copied the model to node-local
disk (MODEL_PATH).
